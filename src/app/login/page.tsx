...
function LoginAccountPage() {
    ...
    const handleclickloginaccount = async (formdata : FormData) => {
       starttransition(async () => {
        const {errorMessage} = await loginAction(formdata);
        if (!errorMessage) {
            router.replace('/')
            toast.success('You are logged in successfly')
        }else {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4 pb-24">
        <div className="bg-popover relative flex w-full max-w-sm flex-col items-center rounded-2xl border border-gray-400 p-8">
            ...
            <form className={`flex w-full flex-col gap-4 ${isPending && "opacity-0 -z-10"}`} action={handleclickloginaccount}>
